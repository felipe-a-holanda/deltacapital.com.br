...
class Proposta(models.Model):
    ...
    valor_comissao_operador = models.DecimalField(
        max_digits=13, decimal_places=2, null=True, blank=True
    )
    valor_comissao_campanha = models.DecimalField(
        max_digits=13, decimal_places=2, null=True, blank=True
    )
    valor_comissao_liquido = models.DecimalField(
        max_digits=13, decimal_places=2, null=True, blank=True
    )

    arquivo_cnh = models.FileField(
        "Arquivo da CNH", upload_to=UploadToPath("cnh"), null=True, blank=True
    )
    arquivo_nf = models.FileField(
        "Dut ou NF", upload_to=UploadToPath("nf"), null=True, blank=True
    )
    arquivo_rg = models.FileField(
        "Arquivo do RG", upload_to=UploadToPath("rg"), null=True, blank=True
    )
    arquivo_cpf = models.FileField(
        "Arquivo do CPF", upload_to=UploadToPath("cpf"), null=True, blank=True
    )
    arquivo_endereco = models.FileField(
        "Arquivo do Comprovante de Endereço",
        upload_to=UploadToPath("endereco"),
        null=True,
        blank=True,
    )
    arquivo_renda = models.FileField(
        "Arquivo do Comprovante de Renda",
        upload_to=UploadToPath("renda"),
        null=True,
        blank=True,
    )
    arquivo_contrato_social = models.FileField(
        "Arquivo do Contrato Social",
        upload_to=UploadToPath("contrato_social"),
        null=True,
        blank=True,
    )
    ...
    proposta_porto = models.OneToOneField(
        PropostaPorto, null=True, blank=True, on_delete=models.SET_NULL
    )
